...
final class ProgressBar
{
    ...
    public function setMaxSteps(int $max)
    {
        $this->format = null;
        $this->max = \max(0, $max);
        $this->stepWidth = $this->max ? \Symfony\Component\Console\Helper\Helper::width((string) $this->max) : 4;
    }
    /**
    ...
    private static function initPlaceholderFormatters() : array
    {
        return ['bar' => function (self $bar, OutputInterface $output) {
            ...
            return \Symfony\Component\Console\Helper\Helper::formatTime(\time() - $bar->getStartTime());
        }, 'remaining' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new LogicException('Unable to display the remaining time if the maximum number of steps is not set.');
            }
            return \Symfony\Component\Console\Helper\Helper::formatTime($bar->getRemaining());
        }, 'estimated' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new LogicException('Unable to display the estimated time if the maximum number of steps is not set.');
            }
            return \Symfony\Component\Console\Helper\Helper::formatTime($bar->getEstimated());
        }, 'memory' => function (self $bar) {
            return \Symfony\Component\Console\Helper\Helper::formatMemory(\memory_get_usage(\true));
    ...
    private function buildLine() : string
    {
        ...
        $linesLength = \array_map(function ($subLine) {
            return \Symfony\Component\Console\Helper\Helper::width(\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), \rtrim($subLine, "\r")));
        }, \explode("\n", $line));
