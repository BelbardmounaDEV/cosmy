...
namespace ps_metrics_module_v4_0_5\PrestaShop\HeaderStamp\Command;

use ps_metrics_module_v4_0_5\PhpParser\Node\Stmt;
use ps_metrics_module_v4_0_5\PhpParser\ParserFactory;
use ps_metrics_module_v4_0_5\PrestaShop\HeaderStamp\LicenseHeader;
use ps_metrics_module_v4_0_5\PrestaShop\HeaderStamp\Reporter;
...
class UpdateLicensesCommand extends Command
{
    ...
    protected function initialize(InputInterface $input, OutputInterface $output) : void
    {
        ...
        $this->folderFilters = $input->getOption('exclude') ? \explode(',', $input->getOption('exclude')) : [];
        $this->fileFilters = $input->getOption('not-name') ? \explode(',', $input->getOption('not-name')) : [];
    ...
    private function findAndCheckExtension(InputInterface $input, OutputInterface $output, string $ext) : void
    {
        ...
        $finder = new Finder();
        $finder->files()->name('*.' . $ext)->in($this->targetDirectory)->exclude($this->folderFilters)->notPath($this->folderFilters)->notName($this->fileFilters);
        ...
        foreach ($finder as $file) {
            switch ($file->getExtension()) {
                case 'php':
                    ...
                    } catch (\ps_metrics_module_v4_0_5\PhpParser\Error $exception) {
    ...
    private function addLicenseToNode(Stmt $node, SplFileInfo $file) : void
    {
        ...
        foreach ($comments as $comment) {
            if ($comment instanceof \ps_metrics_module_v4_0_5\PhpParser\Comment && \strpos($comment->getText(), $this->discriminationString) !== \false) {
    ...
    private function addLicenseToJsonFile(SplFileInfo $file) : bool
    {
        ...
        $content = \json_decode($file->getContents(), \true);
        $oldContent = $content;
