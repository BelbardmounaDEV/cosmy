 ...
 */
namespace ps_metrics_module_v4_0_5\PHP_CodeSniffer\Standards\Squiz\Sniffs\Commenting;
...
class FunctionCommentThrowTagSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        do {
            ...
            */
            $nextToken = $phpcsFile->findNext(\T_WHITESPACE, $currPos + 1, null, \true);
            if ($tokens[$nextToken]['code'] === \T_NEW || $tokens[$nextToken]['code'] === \T_NS_SEPARATOR || $tokens[$nextToken]['code'] === \T_STRING) {
                if ($tokens[$nextToken]['code'] === \T_NEW) {
                    $currException = $phpcsFile->findNext([\T_NS_SEPARATOR, \T_STRING], $currPos, $stackPtrEnd, \false, null, \true);
                } else {
                    $currException = $nextToken;
                }
                if ($currException !== \false) {
                    $endException = $phpcsFile->findNext([\T_NS_SEPARATOR, \T_STRING], $currException + 1, $stackPtrEnd, \true, null, \true);
                    ...
                    if ($catch !== \false) {
                        ...
                        if ($tokens[$thrownVar]['content'] === $tokens[$nextToken]['content']) {
                            ...
                            foreach ($exceptions as $exception) {
                                $thrownExceptions[] = \trim($exception);
                            }
                        }
                    }
                } else {
