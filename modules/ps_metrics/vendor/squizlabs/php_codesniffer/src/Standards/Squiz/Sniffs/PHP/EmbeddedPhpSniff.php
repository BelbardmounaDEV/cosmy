 ...
 */
namespace ps_metrics_module_v4_0_5\PHP_CodeSniffer\Standards\Squiz\Sniffs\PHP;
...
class EmbeddedPhpSniff implements Sniff
{
    ...
    private function validateMultilineEmbeddedPhp($phpcsFile, $stackPtr, $closingTag)
    {
        ...
        if ($tokens[$lastContentBeforeBlock]['line'] === $tokens[$stackPtr]['line'] && \trim($tokens[$lastContentBeforeBlock]['content']) !== '') {
            $error = 'Opening PHP tag must be on a line by itself';
    ...
    private function reportEmptyTagSet(File $phpcsFile, $stackPtr, $closeTag)
    {
        ...
        if ($fix === \true) {
            ...
            if (isset($tokens[$closeTag + 1]) === \true && $tokens[$closeTag + 1]['line'] !== $tokens[$closeTag]['line'] && $tokens[$stackPtr - 1]['code'] === \T_INLINE_HTML && $tokens[$stackPtr - 1]['line'] === $tokens[$stackPtr]['line'] && $tokens[$stackPtr - 1]['column'] === 1 && \trim($tokens[$stackPtr - 1]['content']) === '') {
                $phpcsFile->fixer->replaceToken($stackPtr - 1, '');
            }
            $phpcsFile->fixer->endChangeset();
        }
