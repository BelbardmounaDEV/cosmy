 ...
 * @deprecated 3.9.0
 */
namespace ps_metrics_module_v4_0_5\PHP_CodeSniffer\Standards\Squiz\Sniffs\CSS;
...
class ShorthandSizeSniff implements Sniff
{
     ...
     * @var array
     */
    protected $excludeStyles = ['background-position' => 'background-position', 'box-shadow' => 'box-shadow', 'transform-origin' => 'transform-origin', '-webkit-transform-origin' => '-webkit-transform-origin', '-ms-transform-origin' => '-ms-transform-origin'];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($content !== \trim($matched)) {
            return;
        }
        if ($num === 3) {
            $expected = \trim($content . ' ' . $values[1][0]);
            ...
            if ($fix === \true) {
                ...
                $next = $phpcsFile->findNext(\T_WHITESPACE, $stackPtr + 2, null, \true);
        ...
        if ($values[0][0] === $values[1][0]) {
            ...
            $expected = \trim($values[0][0]);
        } else {
            $expected = \trim($values[0][0]) . ' ' . \trim($values[1][0]);
        }
