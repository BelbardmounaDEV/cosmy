 ...
 */
namespace ps_metrics_module_v4_0_5\PHP_CodeSniffer\Standards\Generic\Sniffs\PHP;
...
class LowerCaseConstantSniff implements Sniff
{
     ...
     */
    private $propertyTypeTokens = [\T_CALLABLE => \T_CALLABLE, T_SELF => T_SELF, T_PARENT => T_PARENT, T_FALSE => T_FALSE, T_TRUE => T_TRUE, T_NULL => T_NULL, \T_STRING => \T_STRING, \T_NAME_QUALIFIED => \T_NAME_QUALIFIED, \T_NAME_FULLY_QUALIFIED => \T_NAME_FULLY_QUALIFIED, \T_NAME_RELATIVE => \T_NAME_RELATIVE, \T_NS_SEPARATOR => \T_NS_SEPARATOR, \T_NAMESPACE => \T_NAMESPACE, T_TYPE_UNION => T_TYPE_UNION, T_TYPE_INTERSECTION => T_TYPE_INTERSECTION, T_NULLABLE => T_NULLABLE];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
         ...
         */
        if (isset(Tokens::$scopeModifiers[$tokens[$stackPtr]['code']]) === \true || $tokens[$stackPtr]['code'] === \T_VAR || $tokens[$stackPtr]['code'] === \T_STATIC || $tokens[$stackPtr]['code'] === \T_READONLY) {
        ...
        if ($tokens[$stackPtr]['code'] === \T_FUNCTION || $tokens[$stackPtr]['code'] === T_CLOSURE || $tokens[$stackPtr]['code'] === \T_FN) {
            ...
            foreach ($params as $param) {
                ...
                for ($i = $param['default_token']; $i < $paramEnd; $i++) {
                    if (isset($this->targets[$tokens[$i]['code']]) === \true) {
