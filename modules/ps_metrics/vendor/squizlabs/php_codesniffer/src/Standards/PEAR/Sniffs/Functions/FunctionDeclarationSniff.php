 ...
 */
namespace ps_metrics_module_v4_0_5\PHP_CodeSniffer\Standards\PEAR\Sniffs\Functions;

use ps_metrics_module_v4_0_5\PHP_CodeSniffer\Files\File;
use ps_metrics_module_v4_0_5\PHP_CodeSniffer\Sniffs\Sniff;
use ps_metrics_module_v4_0_5\PHP_CodeSniffer\Standards\Generic\Sniffs\Functions\OpeningFunctionBraceBsdAllmanSniff;
use ps_metrics_module_v4_0_5\PHP_CodeSniffer\Standards\Generic\Sniffs\Functions\OpeningFunctionBraceKernighanRitchieSniff;
...
class FunctionDeclarationSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_FUNCTION && (isset($tokens[$stackPtr]['scope_opener']) === \true || $methodProps['has_body'] === \false)) {
    ...
    public function processArgumentList($phpcsFile, $stackPtr, $indent, $type = 'function')
    {
        ...
        for ($i = $openBracket + 1; $i < $closeBracket; $i++) {
            ...
            if ($tokens[$i]['code'] === T_OPEN_PARENTHESIS && isset($tokens[$i]['parenthesis_closer']) === \true) {
                $prevNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $i - 1, null, \true);
