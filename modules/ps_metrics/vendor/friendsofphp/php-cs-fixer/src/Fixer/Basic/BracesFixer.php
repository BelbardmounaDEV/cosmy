...
use ps_metrics_module_v4_0_5\PhpCsFixer\Fixer\ControlStructure\ControlStructureContinuationPositionFixer;
use ps_metrics_module_v4_0_5\PhpCsFixer\Fixer\LanguageConstruct\DeclareParenthesesFixer;
...
', ['position_after_functions_and_oop_constructs' => self::LINE_SAME])]);
    ...
    private function fixIndents(Tokens $tokens) : void
    {
        ...
        for ($index = 0, $limit = \count($tokens); $index < $limit; ++$index) {
            ...
            if ($token->isGivenKind($classyTokens) && !$tokensAnalyzer->isAnonymousClass($index)) {
                ...
                if (!$isAnonymousClass && $tokens[$closingParenthesisIndex - 1]->isWhitespace() && \str_contains($tokens[$closingParenthesisIndex - 1]->getContent(), "\n")) {
    ...
    private function ensureWhitespaceAtIndexAndIndentMultilineComment(Tokens $tokens, int $index, string $whitespace) : void
    {
        ...
        if ($nextToken->isComment()) {
            ...
            if ($previousToken->isWhitespace() && 1 === Preg::match('/\\R$/', $previousToken->getContent()) && (\str_starts_with($nextTokenContent, '//' . $this->whitespacesConfig->getIndent()) || '//' === $nextTokenContent || (\str_starts_with($nextTokenContent, '#' . $this->whitespacesConfig->getIndent()) || '#' === $nextTokenContent))) {
                return;
            }
