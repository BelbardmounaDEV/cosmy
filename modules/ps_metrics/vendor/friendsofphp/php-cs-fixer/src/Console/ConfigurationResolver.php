 ...
 */
namespace ps_metrics_module_v4_0_5\PhpCsFixer\Console;
...
use ps_metrics_module_v4_0_5\PhpCsFixer\Cache\FileCacheManager;
use ps_metrics_module_v4_0_5\PhpCsFixer\Cache\FileHandler;
use ps_metrics_module_v4_0_5\PhpCsFixer\Cache\NullCacheManager;
use ps_metrics_module_v4_0_5\PhpCsFixer\Cache\Signature;
use ps_metrics_module_v4_0_5\PhpCsFixer\ConfigInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\ConfigurationException\InvalidConfigurationException;
use ps_metrics_module_v4_0_5\PhpCsFixer\Console\Command\HelpCommand;
use ps_metrics_module_v4_0_5\PhpCsFixer\Console\Report\FixReport\ReporterFactory;
use ps_metrics_module_v4_0_5\PhpCsFixer\Console\Report\FixReport\ReporterInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\Differ\DifferInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\Differ\NullDiffer;
use ps_metrics_module_v4_0_5\PhpCsFixer\Differ\UnifiedDiffer;
use ps_metrics_module_v4_0_5\PhpCsFixer\Finder;
use ps_metrics_module_v4_0_5\PhpCsFixer\Fixer\DeprecatedFixerInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\Fixer\FixerInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\FixerFactory;
use ps_metrics_module_v4_0_5\PhpCsFixer\Linter\Linter;
use ps_metrics_module_v4_0_5\PhpCsFixer\Linter\LinterInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\RuleSet\RuleSet;
use ps_metrics_module_v4_0_5\PhpCsFixer\RuleSet\RuleSetInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\StdinFileInfo;
use ps_metrics_module_v4_0_5\PhpCsFixer\ToolInfoInterface;
use ps_metrics_module_v4_0_5\PhpCsFixer\Utils;
use ps_metrics_module_v4_0_5\PhpCsFixer\WhitespacesFixerConfig;
use ps_metrics_module_v4_0_5\PhpCsFixer\WordMatcher;
    ...
    private function validateRules(array $rules) : void
    {
        ...
        if (\count($unknownFixers) > 0) {
            ...
            foreach ($unknownFixers as $unknownFixer) {
                if (isset($renamedRules[$unknownFixer])) {
                    ...
                    $hasOldRule = \true;
                    $message .= \sprintf('"%s" is renamed (did you mean "%s"?%s), ', $unknownFixer, $renamedRules[$unknownFixer]['new_name'], isset($renamedRules[$unknownFixer]['config']) ? ' (note: use configuration "' . HelpCommand::toString($renamedRules[$unknownFixer]['config']) . '")' : '');
                } else {
        ...
        foreach ($fixers as $fixer) {
            ...
            if (isset($rules[$fixerName]) && $fixer instanceof DeprecatedFixerInterface) {
                ...
                $messageEnd = [] === $successors ? \sprintf(' and will be removed in version %d.0.', Application::getMajorVersion() + 1) : \sprintf('. Use %s instead.', \str_replace('`', '"', Utils::naturalLanguageJoinWithBackticks($successors)));
