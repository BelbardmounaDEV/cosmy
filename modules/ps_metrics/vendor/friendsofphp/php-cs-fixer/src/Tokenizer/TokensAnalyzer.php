 ...
 */
namespace ps_metrics_module_v4_0_5\PhpCsFixer\Tokenizer;

use ps_metrics_module_v4_0_5\PhpCsFixer\Tokenizer\Analyzer\AttributeAnalyzer;
use ps_metrics_module_v4_0_5\PhpCsFixer\Tokenizer\Analyzer\GotoLabelAnalyzer;
/**
...
final class TokensAnalyzer
{
    ...
    public function isBlockMultiline(Tokens $tokens, int $index) : bool
    {
        ...
        for (++$index; $index < $endIndex; ++$index) {
            ...
            if ($token->isWhitespace() && !$tokens[$index - 1]->isGivenKind(\T_END_HEREDOC) && \str_contains($token->getContent(), "\n")) {
                return \true;
            }
        }
        return \false;
    ...
    public function isConstantInvocation(int $index) : bool
    {
        ...
        if ($this->tokens[$nextIndex]->equalsAny(['(', '{']) || $this->tokens[$nextIndex]->isGivenKind([\T_AS, \T_DOUBLE_COLON, \T_ELLIPSIS, \T_NS_SEPARATOR, CT::T_RETURN_REF, CT::T_TYPE_ALTERNATION, CT::T_TYPE_INTERSECTION, \T_VARIABLE])) {
            return \false;
        }
    ...
    {
        static $allowedPrevToken = [']', [\T_STRING], [\T_VARIABLE], [CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE], [CT::T_DYNAMIC_PROP_BRACE_CLOSE], [CT::T_DYNAMIC_VAR_BRACE_CLOSE]];
        $tokens = $this->tokens;
        $token = $tokens[$index];
    ...
    public function isUnaryPredecessorOperator(int $index) : bool
    {
        ...
        if (null === $disallowedPrevTokens) {
            $disallowedPrevTokens = [']', '}', ')', '"', '`', [CT::T_ARRAY_SQUARE_BRACE_CLOSE], [CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE], [CT::T_DYNAMIC_PROP_BRACE_CLOSE], [CT::T_DYNAMIC_VAR_BRACE_CLOSE], [\T_CLASS_C], [\T_CONSTANT_ENCAPSED_STRING], [\T_DEC], [\T_DIR], [\T_DNUMBER], [\T_FILE], [\T_FUNC_C], [\T_INC], [\T_LINE], [\T_LNUMBER], [\T_METHOD_C], [\T_NS_C], [\T_STRING], [\T_TRAIT_C], [\T_VARIABLE]];
        }
        $tokens = $this->tokens;
        $token = $tokens[$index];
